...
function spin_y_correlation!(SySy::AbstractArray{C}, a::Int, b::Int,
    ...
    for l in 0:Lτ
        ...
        contract_G0r_Gr0!(SySy_τ, Gup_βmτ0, Gdn_τ0, b_ba, b_ab, 1, unit_cell, lattice, sgn)
        # SySy(τ,r) = SxSx(τ,r) + 1/N sum_i G₋(b,i,β-τ|a,i+r,0)⋅G₊(a,i+r,τ|b,i,0)
        contract_G0r_Gr0!(SySy_τ, Gdn_βmτ0, Gup_τ0, b_ba, b_ab, 1, unit_cell, lattice, sgn)
    end
...
function spin_y_correlation!(SySy::AbstractArray{C}, a::Int, b::Int,
    ...
    contract_G0r_Gr0!(SySy, Gup, Gdn, b_ba, b_ab, -1, unit_cell, lattice, sgn)
    # SySy(r) = SxSx(r) - 1/N sum_i G₋(b,i|a,i+r)⋅G₊(a,i+r|b,i)
    contract_G0r_Gr0!(SySy, Gdn, Gup, b_ba, b_ab, -1, unit_cell, lattice, sgn)
    # SySy(r) = SySy(r) + 1/N sum_i δ(a,b)⋅δ(r,0)⋅G₊(a,i+r|b,i)
    contract_δGr0!(SySy, Gup, b_ab, b_ab, 1, unit_cell, lattice, sgn)
    # SySy(r) = SySy(r) + 1/N sum_i δ(a,b)⋅δ(r,0)⋅G₋(a,i+r|b,i)
    contract_δGr0!(SySy, Gdn, b_ab, b_ab, 1, unit_cell, lattice, sgn)
